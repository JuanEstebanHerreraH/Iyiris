<script lang="ts">
  ...
  $: baseCmyk = $currentPalette ? rgbToCMYK($currentPalette.base.rgb) : null;
        ...
        {#if $currentPalette}
          <div>
            ...
            <div class="space-y-1.5 font-mono">
              {#each [
                ['HEX',  $currentPalette.base.hex.toUpperCase()],
                ['RGB',  $currentPalette.base.rgb.join(', ')],
                ['HSL',  `${$currentPalette.base.hsl[0]}° ${Math.round($currentPalette.base.hsl[1]*100)}% ${Math.round($currentPalette.base.hsl[2]*100)}%`],
                ['CMYK', baseCmyk ? `${baseCmyk[0]}%, ${baseCmyk[1]}%, ${baseCmyk[2]}%, ${baseCmyk[3]}%` : '—'],
              ] as [lbl, val]}
                <div class="flex justify-between items-center py-2 px-3 rounded-lg bg-bg/60 border border-border gap-2">
                  <span class="text-muted text-xs shrink-0">{lbl}</span>
                  <span class="text-text select-all text-xs text-right">{val}</span>
                </div>
              {/each}
            </div>
          </div>
