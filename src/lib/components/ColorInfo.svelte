<script lang="ts">
  ...
  // Which field is currently focused — prevents overwriting while user types
  let focused: 'hex' | 'rgb' | 'hsl' | 'cmyk' | null = null;

  // ── Sync from parent palette (only when field is not focused) ───────────────
  $: if (palette) syncFromPalette(palette);

  function syncFromPalette(p: ColorPalette) {
    if (focused !== 'hex') {
      hexInput = p.base.hex.toUpperCase();
      hexError = false;
    }
    if (focused !== 'rgb') {
      [rInput, gInput, bInput] = p.base.rgb.map(String);
      rgbError = false;
    }
    if (focused !== 'hsl') {
  ...
  function onHexInput(e: Event) {
    ...
    if (isValidHex(hex)) {
      hexError = false;
      dispatch('change', hex.toLowerCase());
    } else {
      hexError = true;
    }
  }
  ...
  function onRgbInput() {
    ...
    if ([r, g, b].every(n => !isNaN(n) && n >= 0 && n <= 255)) {
      rgbError = false;
      try { dispatch('change', chroma(r, g, b).hex()); }
      catch { rgbError = true; }
    } else {
      rgbError = true;
    }
  ...
  function onHslInput() {
    ...
    const s = parseFloat(sInput) / 100;
    const l = parseFloat(lInput) / 100;
    if (!isNaN(h) && !isNaN(s) && !isNaN(l) &&
        h >= 0 && h <= 360 && s >= 0 && s <= 1 && l >= 0 && l <= 1) {
      hslError = false;
      try { dispatch('change', chroma.hsl(h, s, l).hex()); }
      catch { hslError = true; }
    } else {
      hslError = true;
    }
  ...
  function onCmykInput() {
    const c = parseFloat(cInput), m = parseFloat(mInput);
    const y = parseFloat(yInput), k = parseFloat(kInput);
  ...
  <div class="space-y-1.5 font-mono">
    ...
    <div class="color-row" class:error={rgbError}>
      ...
      <div class="flex gap-1 flex-1 justify-end">
        {#each [
          { bind: rInput, set: (v: string) => (rInput = v), label: 'R', max: 255 },
          { bind: gInput, set: (v: string) => (gInput = v), label: 'G', max: 255 },
          { bind: bInput, set: (v: string) => (bInput = v), label: 'B', max: 255 },
        ] as ch}
          <div class="relative">
            <input
              class="channel-input"
              ...
              max={ch.max}
              value={ch.bind}
              on:input={(e) => { ch.set((e.target as HTMLInputElement).value); onRgbInput(); }}
              on:focus={() => (focused = 'rgb')}
              on:blur={() => (focused = null)}
              title={ch.label}
            />
          </div>
    ...
    <div class="color-row" class:error={hslError}>
      ...
      <div class="flex gap-1 flex-1 justify-end items-center">
        <div class="relative">
          <input
            class="channel-input w-14"
            type="number" min="0" max="360"
            bind:value={hInput}
            on:input={onHslInput}
            on:focus={() => (focused = 'hsl')}
            on:blur={() => (focused = null)}
            title="Hue"
          />
    ...
    <div class="color-row" class:error={cmykError}>
      ...
      <div class="flex gap-1 flex-1 justify-end items-center">
        {#each [
          { val: cInput, set: (v: string) => (cInput = v),  title: 'Cyan' },
          { val: mInput, set: (v: string) => (mInput = v),  title: 'Magenta' },
          { val: yInput, set: (v: string) => (yInput = v),  title: 'Yellow' },
          { val: kInput, set: (v: string) => (kInput = v),  title: 'Key (Black)' },
        ] as ch}
          <div class="relative">
            <input
              class="channel-input w-12"
              type="number" min="0" max="100"
              value={ch.val}
              on:input={(e) => { ch.set((e.target as HTMLInputElement).value); onCmykInput(); }}
              on:focus={() => (focused = 'cmyk')}
              on:blur={() => (focused = null)}
              title={ch.title}
            />
            <span class="unit">%</span>
...
<style>
  ...
  .channel-input {
    ...
    padding-right: 1rem; /* space for unit label */
  }
  ...
  .unit {
    ...
    font-size: 0.65rem;
    color: var(--color-muted, rgb(255 255 255 / 0.35));
    pointer-events: none;
  }
</style>
